...
class Twitter {
   ...
   constructor(input = null) {
      this.#input = input;
      this.#cache = new NodeCache({ deleteOnExpire: true });
   ...
   async download(link = this.#input) {
      try {
         ...
         if (!data.tweetResult.hasOwnProperty('result')) return { status: false, message: 'Result empty.' };
         ...
         const result = {
            ...
            views_count: this.#parseCount(mediaMetadata.views?.count || mediaMetadata.legacy.extended_entities.media[0]?.mediaStats?.viewCount, 'Views'),
            data: mediaMetadata.legacy.extended_entities.media.map((media) => {
               if (media.type === 'photo') {
                  return {
                     type: media.type,
   ...
   async getPostUser(username = this.#input) {
      try {
         ...
         const response = await fetch(encodeURI(url), { method: 'GET', headers: { 'authorization': `Bearer ${token.tokenBearer}`, 'x-guest-token': token.tokenGuest } }).then((response) => response.json());
   ...
   async stalker(username = this.#input) {
      try {
         ...
         if (dataJson.data.errors) return { status: false, message: dataJson.data.errors[0].message };
